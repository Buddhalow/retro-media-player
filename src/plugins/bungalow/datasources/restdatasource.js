import SPDataSource from '@/controls/datasource.js';

export default class SPRestDataSource extends SPDataSource {
  ...
  request(method, uri, options, data) {
    var url = '/api/'
    if (!options) {
      options = {}
    }
    ...
    let overlay = null;
    try {
      overlay = fetch('/api/overlay/?uri=' + encodeURIComponent(uri), {
        method: method,
