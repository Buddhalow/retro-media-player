...
class Store extends EventEmitter {
    ...
    getCurrentTrack() {
        let result = this.request('GET', 'spotify:me:player:currently-playing', null, null, false);
    ...
    getAlbumById(id) {
        ...
        let result = fetch('/api/album/' + id, {credentials: 'include', mode: 'cors'}).then((e) => e.json())
        this.setState(uri, result);
        return result;
    }
    getArtistById(id) {
        let uri = 'spotify:artist:' + id;
        let result = fetch('/api/artist/' + id, {credentials: 'include', mode: 'cors'}).then((e) => e.json());
        this.setState(uri, result);
        return result;
    }
    login() {
...
class SPAppHeaderElement extends HTMLElement {
    attachedCallback() {
        if (!this.created) {
            this.innerHTML = '<button id="btnBack" class="fa fa-arrow-left" onclick="history.back()"><button class="fa fa-arrow-right" onclick="history.forward()"></button>';
            if (!this.searchForm) {
...
class SPHeaderElement extends SPResourceElement {
    ...
    render(object) {
        ...
        object.buttons.map((button) => {
            ...
            btn.innerHTML = '<i class="fa fa-' + button.icon + '"></i> ' + button.label;
...
class SPAqtivityListViewElement extends SPViewElement {
    attachedCallback() {
        if (!this.created) {
            ...
            this.listView.dataSource = new SPAqtivityParseTableDataSource('Aqtivity', ['status', 'facility', 'sport' , 'time', 'roaming', 'cost', 'dimension']);
...
document.currentScript.ownercustomElements.define('sp-popularity', PopularityBar);
...
document.currentScript.ownercustomElements.define('sp-contextmenu', ContextMenu);
...
class SPFungalifyViewElement extends SPViewElement {
    attachedCallback() {
        if (!this.created) {
            ...
            this.listView.dataSource = new SPParseTableDataSource('FungalTreatment', ['time']);
            this.listView.fetchNext();
            this.listView.header = this.header;
            this.listView.view = this;
            this.created = true;
...
class SPOpportunifyViewElement extends SPViewElement {
    attachedCallback() {
        if (!this.created) {
            ...
            this.appendChild(this.listView);
            this.listView.designer = new SPParseTableDesigner();
            this.listView.dataSource = new SPParseTableDataSource('Opportunity', ['status', 'name', 'time']);
            this.listView.fetchNext();
            this.listView.header = this.header;
            this.listView.view = this;
            this.created = true;
