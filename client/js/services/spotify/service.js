    ...
    player.addListener(
      ...
      ({
        ...
        try {
          ...
          $('tr[data-uri="' + this.state.player.item.uri + '"]').addClass(
            "sp-current-track"
          );
          ...
          if (
            ...
            $("sp-nowplaying .nowplayingimage").attr(
              "data-uri",
              this.state.player.item.album.uri
            );
            $("sp-nowplaying .nowplayingimage").css({
              "background-image":
                'url("' + this.state.player.item.album.images[0].url + '")',
            });

            $("sp-nowplaying .nowplayingtitle").html(`
                      <sp-link uri="${this.state.player.item.uri}">${
              this.state.player.item.name
            }</sp-link><br>
                      ${this.state.player.item.artists
                        .map(
                          (a) => `
