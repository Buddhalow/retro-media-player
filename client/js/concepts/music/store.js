...
import musicService from '/js/plugins/music/service.js';
...
class Store extends EventEmitter {
  onmusicWebPlaybackSDKReady = (player) => {
  ...
  async getDevices() {
    try {
      let result = await this.request('GET', 'music:me:player:devices', null, null, false, true);
  ...
  async playPause() {
    ...
    if (this.state.player.is_playing) {
      result = this.request('PUT', 'music:me:player:pause', {device_id: device.id}, null, false, true);
    } else {
      result = this.request('PUT', 'music:me:player:play', {device_id: device.id}, null, false, true);
    }
    this.state.player = await this.getCurrentTrack();
    this.emit('change');
  }
  ...
  seek(pos) {
    this.request('PUT', 'music:me:player:seek', null, {position_ms: pos})
  }
  ...
  async play(context) {
    let result = this.request('PUT', 'music:me:player:play', {}, context, false);
    this.state.player = await this.getCurrentTrack();
    this.emit('change');
  ...
  async playTrack(track, context) {
    ...
    this.request('PUT', 'music:me:player:play', {device_id: device.id}, payload, false, true);
    this.hasStartedPlaying = true
    this.currentDevice = device;
  }

  playTrackAtPosition(position, context) {
    this.request('PUT', 'music:me:player:play', {device_id: device.id}, {
  ...
  async getCurrentTrack() {
    let result = await this.request('GET', 'music:me:player:currently-playing', null, null, false, true);
  ...
  async request(method, uri, params, payload, cache = true, direct = false) {
    ...
    try {
      ...
      if (uri == null) return;
      var url = uri;
      if (uri.indexOf('bungalow:') === 0 || uri.indexOf('music:') === 0) {
        let parts = url.split(':').slice(1);
        if (direct) {
          if (parts[0] === 'search') {
            let session = JSON.parse(localStorage.getItem('music.session'))
            return await fetch('https://api.music.com/v1/search?q=' + parts[1] + '&type=' + parts[2] + '&' + query, {
              mode: 'cors',
              headers: {
          ...
          url = 'https://api.music.com/v1/' + parts.join('/')

          if (parts[0] === 'playlists') {
            if (parts[2] === 'snapshot' && parts[4] === 'tracks') {

              url = 'https://api.music.com/v1/playlists/' + parts[1] + '/tracks?' + query;
            }
          }
          if (parts[0] === 'artists') {
            if (parts.length > 2) {
              if (parts[2] === 'singles') {
                url = 'https://api.music.com/v1/artists/' + parts[1] + '/albums?album_type=single&' + query;
              }
              if (parts[2] === 'albums') {
                url = 'https://api.music.com/v1/artists/' + parts[1] + '/albums?album_type=album&' + query;
              }
              ...
              if (parts[2] === 'tops') {
                if (parts.length > 4) {
                  let result = await this._request('GET', 'music:artist:' + parts[1] + ':track')
                  return result;
                } else if (parts.length > 3) {
                  return {
                    id: 'toplist',
                    name: 'Top List',
                    uri: 'music:artist:' + parts[1] + ':top:' + parts[3],
                    type: 'toplist',
                    'for': {
                      name: 'Artist',
                      id: parts[1],
        ...
        if (method === 'GET') {
          ...
          result = await fetch(url + '?' + query, {
            ...
            headers: {
              'Content-Type': 'application/json',
              'Authorization': 'Bearer ' + JSON.parse(localStorage.getItem('music.session')).access_token
            },
            method: method,
            body: JSON.stringify(payload)
          }).then((e) => {

            if ((e.status < 200 || e.status > 299) && uri.indexOf('music:me:player:play') !== -1) {
              alert(e.status);
            }
        ...
        if (result && 'objects' in result) {
          ...
          if (!result.uri) {
            result.uri = 'music:player'; // TODO Fix this later
          }
        ...
        try {
          ...
          if (storeNode.uri?.indexOf('music:') !== 0) {
            storeNode = {
            ...result,
                uri: `music:${result.uri}`
            }
          }
   ...
   **/
  getAlbumById(id) {
    let uri = 'music:album:' + id;
    let result = fetch('/api/music/album/' + id, {credentials: 'include', mode: 'cors'}).then((e) => e.json())
    this.setState(uri, result);
    return result;
  }

  getArtistById(id) {
    let uri = 'music:artist:' + id;
    let result = fetch('/api/music/artist/' + id, {credentials: 'include', mode: 'cors'}).then((e) => e.json());
    this.setState(uri, result);
    return result;
  }
  ...
  login() {
    return new Promise((resolve, reject) => {
      var loginWindow = window.open('/api/music/music/login');
