import SPPlaylistListElement from '/js/concepts/music/components/playlistlist.js';
import SPSearchHeaderElement from '/js/concepts/music/elements/searchheader.js';
import SPPlaylistContextElement from '/js/concepts/music/elements/playlistcontext.js';
import SPShowContextElement from '/js/concepts/music/elements/showcontext.js';
import SPTrackContextElement from '/js/concepts/music/elements/trackcontext.js';
import SPEpisodeContextElement from '/js/concepts/music/elements/episodecontext.js';
import SPResourceContextElement from '/js/concepts/music/elements/resourcecontext.js';
import SPPlaylistElement from '/js/concepts/music/elements/playlist.js';
import SPShowElement from '/js/concepts/music/elements/show.js';
import SPTrackControlsElement from '/js/concepts/music/elements/trackcontrols.js';
import SPAboutArtistElement from '/js/concepts/music/elements/aboutartist.js';
import SPAudioBookElement from '/js/concepts/music/elements/audiobook.js';
import SPAudioBookContextElement from '/js/concepts/music/elements/audiobookcontext.js';
import SPTopListElement from '/js/concepts/music/elements/toplist.js';
import SPMenuDataSource from '/js/controls/menudatasource.js';
import store from '/js/concepts/music/player.js';

customElements.define('sp-playlist', SPPlaylistElement);
customElements.define('sp-show', SPShowElement);
customElements.define('sp-musicsearchheader', SPSearchHeaderElement);
...
customElements.define('sp-musicaboutartist', SPAboutArtistElement);
...
document.addEventListener('viewload', async (e) => {
    if (/^bungalow:internal:start$/.test(e.detail.uri)) {
        /* var categoriesTab = e.detail.addTab('categories', '<i class="fa fa-music"></i> Categores');
        ...
        categoriesTab.innerHTML = '<sp-flow uri="music:category"></sp-flow>'
        var genresTab = e.detail.addTab('genres', '<i class="fa fa-music"></i> Genres');
        ...
        genresTab.innerHTML = '<div class="container"><sp-resourcecontext showcolumnheaders="true" fields="name" uri="music:genre"></sp-table></div>'*/
...
document.addEventListener('mainmenuload', async (e) => {
    ...
    const playlists = await store.request('GET', 'music:library:playlists')
...
document.addEventListener('hook_startview', (e) => {
    ...
    let view = document.createElement('sp-musicstartview');
...
document.addEventListener('hook_searchview', (e) => {
    if (e.data instanceof Object) {
        ...
        let search = hook.querySelector('sp-musicsearchview[uri="' + e.data.uri + '"]');
        if (!search) {
            search = document.createElement('sp-musicsearchview');
...
document.addEventListener('hook_startview', async (e) => {
    ...
    let result = await store.request('GET', 'music:browse:new-releases', null, null, false, true);
    let result2 = await store.request('GET', 'music:me:top:artist', null, null, false, true);
    elm.innerHTML = `
...
document.addEventListener('viewload', (e) => {
    if (/^bungalow:search:(.*)$/.test(e.detail.uri)) {
        var tab = e.detail.addTab('music-search', '<i class="fa fa-music"></i> Tracks');
        var container = document.createElement('div');
        container.style.display = 'flex';
        container.style.width = '100%';
        container.style.height = '100%';
        ...
        let uri = 'music:' + (e.detail.uri.split(/:/g).slice(1).join(':'))
        let search = container.querySelector('music-searchview[uri="' + uri + '"]');
        if (!search) {
            search = document.createElement('music-searchview');
            container.appendChild(search);
            search.setAttribute('uri', 'music:' + (e.detail.uri.split(/:/g).slice(1).join(':')));
            $(search).show();
        } else {
        }
...
document.addEventListener('hook_album', (e) => {
    if (e.data instanceof Object) {
        ...
        let search = hook.querySelector('music-searchview[uri="' + e.detail.uri + '"]');
        if (!search) {
            search = document.createElement('music-searchview');
            hook.appendChild(search);
