...
export default class SPChartViewElement extends SPViewElement {
    ...
    connectedCallback() {
        ...
        if (!this.created2) {
            ...
            this.trackcontext.setAttribute('columnheaders', '_,name,artists,duration,album,added_by,added_at');
    ...
    acceptsUri(uri) {
        return /^music:chart:([a-zA-Z0-9]+)$/.test(uri);
    }
    activate() {
        super.activate();
        if (this.invalid) {
    ...
    async setUri(newVal) {
        ...
        this.trackcontext.setAttribute('uri', newVal + ':snapshot:' + (result.snapshot_id ? result.snapshot_id.replace(/\//g, '') : '0') + ':track');
